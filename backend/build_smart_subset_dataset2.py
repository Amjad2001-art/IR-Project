...
BACKUP_DIR = os.path.join(SAVE_DIR, "backup_dataset2_before_smart_subset")

DATASET_NAME = "beir/quora/test"

DEFAULT_SUBSET_SIZE = 3000
...
def get_current_subset_size():
    ...
    if os.path.exists(work_dataset_path):
        try:
            ...
            if current_size > 0:
                print(f"Current Dataset 2 subset size found: {current_size}")
                return current_size
        except Exception:
            pass

    print(f"No valid old Dataset 2 subset found. Using default size: {DEFAULT_SUBSET_SIZE}")
...
def build_smart_documents_subset(dataset, required_doc_ids, subset_size):
    selected_docs = []
    selected_doc_ids = set()

    print("Collecting relevant qrels documents first...")
    ...
    print("Filling remaining documents...")
...
def rebuild_dataset2_artifacts(docs_df):
    ...
    print("Saving Dataset 2 smart subset artifacts...")
...
def main():
    ...
    subset_size = get_current_subset_size()
    ...
    rebuild_dataset2_artifacts(docs_df)

    print("Smart subset for Dataset 2 is ready.")
